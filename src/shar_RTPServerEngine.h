#ifndef _SHAR_RTP_SERVER_ENGINE_H_
#define _SHAR_RTP_SERVER_ENGINE_H_
...
class Caudio;
class EngineServerJTT
{
public:
    EngineServerJTT(const CONFIG ServerConfig, int fd);
    ~EngineServerJTT(); //virtual
...
private:
    ...
    SAVER_RECV_DATA_SIM6*  RecvRtpPackStr;
...
inline void EngineServerJTT::read_packHead_ptr()
{
	dataPtr= nullptr;
	dataPtr = RecvRtpPackStr->HeadPack + PackHeadLen_;
}

inline void EngineServerJTT::read_packHeadAfter_ptr()
{
	dataPtr= nullptr;
