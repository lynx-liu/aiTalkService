...
bool CRTPServerEngine::videAudioManage()
{
	...
	if(DataType4_ == DATA_TYPE_AUDIO && RecvRtpPackStr->PKG_HEADER.PT7 == LOAD_TYPE_G711A\
...
void CRTPServerEngine::get_device_SIM()
{
	m_BCDSIMStr.clear();

	char temp[3] = {0};
	const uint8_t* m_BCDSIMCard = RecvRtpPackStr->PKG_HEADER.BCDSIMCardNumber;
	for(int i = 0; i<m_BCDSIMLength; i++){
		sprintf(temp, "%02X", m_BCDSIMCard[i]);
		m_BCDSIMStr += temp;
	}
}
...
void CRTPServerEngine::head_analysis_()
{
	...
	if(PackHeadLen_ == m_HeadLen){
		...
		if(DataType4_ == DATA_TYPE_AUDIO) PackStatus_ = _PKG_HD_REMAINING_INIT_1;
		else if(DataType4_ == DATA_TYPE_TRANSM) PackStatus_ = _PKG_HD_REMAINING_INIT_2;
